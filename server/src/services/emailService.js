...
function obtenirTransportadorCorreu() {
...
function usaRelayCorreuAplicacio() {
  return Boolean(process.env.EMAIL_RELAY_URL && process.env.EMAIL_RELAY_SECRET);
}

async function enviarCorreuViaRelay({ to, subject, text, html }) {
...
async function enviarCorreuAplicacio({ to, subject, text, html }) {
  if (usaRelayCorreuAplicacio()) {
    return enviarCorreuViaRelay({ to, subject, text, html });
  }

  const transporter = obtenirTransportadorCorreu();
  const from = process.env.EMAIL_FROM || process.env.EMAIL_SMTP_USER;
  return transporter.sendMail({ from, to, subject, text, html });
}

function renderitzarCorreuAnimeWl({ title, intro, bodyHtml, buttonLabel = null, buttonUrl = null, footer }) {
  return `
...
async function enviarCorreuVerificacio(to, code) {
...
}export { enviarCorreuVerificacio };

async function enviarCorreuCanviNomUsuariAdmin(to, newUsername) {
  const safeUsername = escaparHtml(newUsername);
...
}export { enviarCorreuCanviNomUsuariAdmin };

async function enviarCorreuRestablimentContrasenya(to, resetToken) {
...
}export { enviarCorreuRestablimentContrasenya };