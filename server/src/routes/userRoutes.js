...
import { trobarFavoritsPerUsuari, afegirFavorit, eliminarFavorit, actualitzarEstatFavorit, trobarFavoritsPublicsPerUsuari } from '../models/favorites_model.js';
import {
  registrarUsuari,
  trobarUsuariPerId,
  trobarUsuariPerNom,
...
import {
  ...
  validarCorreuElectronic } from
'../utils/auth.js';
import { enviarCorreuCanviNomUsuariAdmin, enviarCorreuRestablimentContrasenya, enviarCorreuVerificacio } from '../services/emailService.js';
  ...
  router.get('/api/session', async (req, res) => {
    try {
      ...
      if (sessionUser.id_anime_preferit == null || sessionUser.id_anime_recomanat == null || sessionUser.img_url == null || sessionUser.isAdmin == null) {
        const result = await trobarUsuariPerNom(sessionUser.nom);
        if (result.error) {
