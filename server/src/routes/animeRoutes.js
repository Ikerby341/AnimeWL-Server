...
import { sincronitzarAnimePerId, sincronitzarMetadadesAnimePerId, maparJikanABaseDades } from '../controllers/syncAnime.js';
import { trobarAnimePerId, llistarAnimesEnEmissio, llistarAnimes, llistarAnimesRecomanatsAleatorisUsuari, obtenirNombreEpisodisPerAnime, llistarGeneres } from '../models/anime_model.js';
import { trobarComentarisPerIdAnime, inserirComentari, eliminarComentariPerId } from '../models/comment_model.js';
import { trobarResumValoracionsPerIdAnime, trobarValoracioPerAnimeIUsuari, desarValoracio } from '../models/rating_model.js';
import { trobarProgresPerAnimeIUsuari, desarProgres, calcularMinutsVistosPerAnime } from '../models/progress_model.js';
import { trobarUsuariPerId } from '../models/users_model.js';
import { obtenirAnimeDeBaseDades, cercarJikan } from '../services/jikanService.js';
import { obtenirIdUsuari } from '../utils/auth.js';

const COMMENT_MAX_LENGTH = 255;

function crearRouterAnime() {
  const router = Router();
  ...
  router.get('/api/genres', async (req, res) => {
    try {
      const genres = await llistarGeneres();
  ...
  router.get('/api/anime/recent/:limit', async (req, res) => {
    ...
    try {
      const anime = await llistarAnimes(null, parseInt(limit));
  ...
  router.get('/api/anime/recommended-random/:limit', async (req, res) => {
    const { limit } = req.params;
    try {
      const anime = await llistarAnimesRecomanatsAleatorisUsuari(parseInt(limit));
      res.json({ success: true, anime });
    } catch (err) {
  ...
  router.get('/api/anime/airing/:limit', async (req, res) => {
    const { limit } = req.params;
    try {
      const anime = await llistarAnimesEnEmissio(limit);
      res.json({ success: true, anime });
    } catch (err) {
  ...
  router.get('/api/anime/genre/:genreId', async (req, res) => {
    ...
    try {
      const anime = await llistarAnimes(genreId);
      res.json({ success: true, anime });
    } catch (err) {
  ...
  router.get('/api/anime/genre/:genreId/:limit', async (req, res) => {
    ...
    try {
      const anime = await llistarAnimes(genreId, limit);
      res.json({ success: true, anime });
    } catch (err) {
  ...
  router.get('/api/jikan/search', async (req, res) => {
    ...
    try {
      data = await cercarJikan(trimmedQuery);
      if (data.length > 0) {
    ...
    data = await obtenirAnimeDeBaseDades(trimmedQuery);
    if (data.length > 0) {
      return res.json({ success: true, data });
    }
  ...
  router.get('/api/anime/:id', async (req, res) => {
    ...
    try {
      let anime = await trobarAnimePerId(id);
      if (anime) {
        anime = await trobarAnimePerId(id);
        res.json({ success: true, anime });
        if (!cacheOnly) {
          sincronitzarAnimePerId(id).catch((error) => console.error('background sync error', error));
        }
        return;
      }
      ...
      let rec;
      try {
        rec = await sincronitzarMetadadesAnimePerId(id);
      } catch (error) {
        console.error('metadata sync error', error);
        try {
          const response = await axios.get(`https://api.jikan.moe/v4/anime/${id}/full`);
          if (response.data && response.data.data) {
            rec = maparJikanABaseDades(response.data.data);
          }
        } catch (jikanError) {
  ...
  router.get('/api/anime/:id/comments', async (req, res) => {
    const { id } = req.params;
    try {
      const comments = await trobarComentarisPerIdAnime(id);
  ...
  async function gestionarEliminacioComentari(req, res) {
    ...
    try {
      ...
      const deleted = await eliminarComentariPerId(commentId, userId, isAdmin);
      if (!deleted) {
  ...
  router.delete('/api/anime/:id/comments/:commentId', gestionarEliminacioComentari);
  router.delete('/api/comments/:commentId', gestionarEliminacioComentari);

  router.get('/api/anime/:id/rating', async (req, res) => {
    const { id } = req.params;
    try {
      const rating = await trobarResumValoracionsPerIdAnime(id);
      let userRating = null;
      if (req.session.user) {
        const userRatingRecord = await trobarValoracioPerAnimeIUsuari(id, req.session.user.id_usuari);
  ...
  router.get('/api/anime/:id/progress', async (req, res) => {
    ...
    try {
      let progress = null;
      if (req.session.user) {
        progress = await trobarProgresPerAnimeIUsuari(id, req.session.user.id_usuari);
      }
      const episodeCount = await obtenirNombreEpisodisPerAnime(id);
  ...
  router.post('/api/anime/:id/progress', async (req, res) => {
    ...
    try {
      const totalMinutes = await calcularMinutsVistosPerAnime(id, chaptersWatched);
      const savedProgress = await desarProgres({
  ...
  router.post('/api/anime/:id/rating', async (req, res) => {
    ...
    try {
      const savedRating = await desarValoracio({
  ...
  router.post('/api/anime/:id/comments', async (req, res) => {
    ...
    try {
      ...
      const inserted = await inserirComentari(newComment);
  ...
  router.post('/api/anime/sync/:id', async (req, res) => {
    const { id } = req.params;
    try {
      const rec = await sincronitzarAnimePerId(id);
