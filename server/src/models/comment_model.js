...
async function trobarComentarisPerIdAnime(id_anime) {
  if (!id_anime) return [];
...
}export { trobarComentarisPerIdAnime };

async function inserirComentari(comment) {
  const { data, error } = await supabase.
  from('comentari').
  insert(comment).
  select('id_comentari, id_usuari, id_anime, id_capitol, contingut, data_hora, usuari(nom, img_url)').
...
}export { inserirComentari };

async function eliminarComentariPerId(id_comentari, id_usuari, isAdmin = false) {
  let query = supabase.
  from('comentari').
  delete().
