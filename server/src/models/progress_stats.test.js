...
import {
  calcularCapitolsTotals,
  calcularMinutsTotals,
  comptarAnimesAcabats,
  sumarMinutsCapitols } from
'./progress_stats.js';
...
test('sumChapterMinutes suma solo los capitulos vistos', () => {
  ...
  assert.equal(sumarMinutsCapitols(chapterRows, 2), 48);
});
...
test('sumChapterMinutes ignora duraciones no numericas', () => {
  ...
  assert.equal(sumarMinutsCapitols(chapterRows, 3), 49);
});
...
test('calculateTotalMinutes suma minuts_totals del progreso', () => {
  ...
  assert.equal(calcularMinutsTotals(progressRows), 168);
});
...
test('calculateTotalChapters suma capitols_vistos del progreso', () => {
  ...
  assert.equal(calcularCapitolsTotals(progressRows), 14);
});
...
test('countFinishedAnimes cuenta solo animes completos', () => {
  ...
  assert.equal(comptarAnimesAcabats(progressRows, episodeCounts), 1);
});