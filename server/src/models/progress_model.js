...
import { obtenirNombreEpisodisPerAnime } from './anime_model.js';
import { calcularCapitolsTotals, calcularMinutsTotals, comptarAnimesAcabats, sumarMinutsCapitols } from './progress_stats.js';

async function calcularMinutsVistosPerAnime(id_anime, capitols_vistos) {
  ...
  return sumarMinutsCapitols(chapterRows || [], chaptersWatched);
}export { calcularMinutsVistosPerAnime };

async function trobarProgresPerAnimeIUsuari(id_anime, id_usuari) {
  ...
  return data || null;
}export { trobarProgresPerAnimeIUsuari };

async function desarProgres(progress) {
...
async function obtenirEstadistiquesUsuari(id_usuari) {
  if (!id_usuari) return {
    totalMinutes: 0,
    totalChapters: 0,
  ...
  await Promise.all(
    animeIds.map(async (animeId) => {
      try {
        episodeCounts[animeId] = await obtenirNombreEpisodisPerAnime(animeId);
      } catch (err) {
  ...
  const totalMinutes = calcularMinutsTotals(progresRows);
  const totalChapters = calcularCapitolsTotals(progresRows);
  ...
  const finishedAnimeCount = comptarAnimesAcabats(progresRows, episodeCounts);
...
}export { obtenirEstadistiquesUsuari };